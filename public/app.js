...
// Boissons 33 cl incluses dans la formule
const DRINKS = ['Coca-Cola','Oasis Pomme Cassis Framboise','Oasis Tropical','Perrier','Ice Tea','Capri-Sun'];

// Viandes au choix (affiche « LES VIANDES »)
const VIANDES_TACOS = [
  { name:'Poulet mariné', price:0 },{ name:'Poulet crème', price:0 },
  { name:'Steak frais', price:0 },{ name:'Kebab', price:0 },
  { name:'Tenders', price:0 },{ name:'Nuggets', price:0 }
];
const VIANDES_ASSIETTE = [
  { name:'Kebab', price:0 },{ name:'Steak frais', price:0 },
  { name:'Poulet crème', price:0 },{ name:'Poulet mariné', price:0 }
];

// Suppléments (affiche « NOS SUPPLÉMENTS »)
const SUPPS_LO = [
  { name:'Cheddar', price:1.00 },{ name:'Raclette', price:1.00 },
  { name:'Œuf', price:1.00 },{ name:'Bacon', price:1.00 },
  { name:'Feta', price:2.00 },
  { name:'Steak frais', price:2.50 },{ name:'Kebab', price:2.50 }
];
const SUPPS_TACOS = [
  { name:'Cheddar', price:1.00 },{ name:'Raclette', price:1.00 },
  { name:'Œuf', price:1.00 },{ name:'Bacon', price:1.00 },
...
// Consts héritées du moteur (modals génériques) — non utilisées ici
const VIANDES = VIANDES_TACOS.map(v=>v.name);
const SAUCES = SAUCES_LO;
const GRATINAGES = [];
const CRUDITES = ['Salade','Tomates','Oignons','Salade+Tomates+Oignons','Sans crudités'];

const MENU = [
  // ── Nos Sandwichs (en formule frites maison + boisson 33 cl : +3 €) ──
  { id:'sandwichs-kebab', cat:'sandwichs', name:'Kebab', desc:'Pain boulanger, kebab de veau, salade, tomates, oignons et sauce au choix.', price:8.00, emoji:'🥙', img:'sandwichs-kebab.jpg' },
  { id:'sandwichs-durum', cat:'sandwichs', name:'Durum', desc:'Galette, kebab de veau/dinde, salade, tomates, oignons et sauce au choix.', price:8.50, emoji:'🌯', img:'sandwichs-durum.jpg' },
  { id:'sandwichs-americain', cat:'sandwichs', name:'Américain', desc:'Pain baguette boulangère, 2 steaks frais, cheddar, frites, salade, tomates, oignons et sauce au choix.', price:8.00, emoji:'🥖', img:'sandwichs-americain.jpg', tag:'Populaire', tagClass:'tag-top' },
  { id:'sandwichs-marine', cat:'sandwichs', name:'Mariné', desc:'Pain baguette boulangère, poulet mariné, cheddar, salade, tomates, oignons et sauce au choix.', price:8.00, emoji:'🥖', img:'sandwichs-marine.jpg' },
  { id:'sandwichs-cremeux', cat:'sandwichs', name:'Crémeux', desc:'Pain baguette boulangère, poulet crème, salade, tomates, oignons et sauce au choix.', price:8.50, emoji:'🥖', img:'sandwichs-cremeux.jpg' },
  { id:'sandwichs-mixte', cat:'sandwichs', name:'Mixte', desc:'Pain baguette boulangère, 2 viandes au choix, salade, tomates, oignons et sauce au choix.', price:9.50, emoji:'🥖', img:'sandwichs-mixte.jpg' },
  { id:'sandwichs-algerien', cat:'sandwichs', name:'Algérien', desc:'Pain baguette boulangère, steak frais, frites, omelette maison, salade, tomates, oignons et sauce au choix.', price:8.50, emoji:'🥖', img:'sandwichs-algerien.jpg' },
  { id:'sandwichs-mexicain', cat:'sandwichs', name:'Méxicain', desc:'Pain baguette boulangère, poulet mariné, poivrons, olives, cheddar, salade, tomates, oignons et sauce au choix.', price:9.00, emoji:'🌶️', img:'sandwichs-mexicain.jpg' },

  // ── Nos Burgers (en formule frites maison + boisson 33 cl : +3 €) ──
  { id:'burgers-simple', cat:'burgers', name:'Simple', desc:'1 steak frais, cheddar, salade, tomates, oignons et sauce au choix.', price:6.00, emoji:'🍔', img:'burgers-simple.jpg' },
  { id:'burgers-double', cat:'burgers', name:'Double', desc:'2 steaks frais, cheddar, salade, tomates, oignons et sauce au choix.', price:8.00, emoji:'🍔', img:'burgers-double.jpg', tag:'Populaire', tagClass:'tag-top' },
  { id:'burgers-rosti-bacon', cat:'burgers', name:'Rösti Bacon', desc:'1 steak frais, cheddar, rösti, bacon, salade, tomates, oignons et sauce au choix.', price:8.00, emoji:'🍔', img:'burgers-rosti-bacon.jpg' },
  { id:'burgers-kebab', cat:'burgers', name:'Burger Kebab', desc:'Lamelles de kebab, cheddar, salade, tomates, oignons et sauce au choix.', price:6.00, emoji:'🍔', img:'burgers-kebab.jpg' },

  // ── Tacos (frites maison + sauce fromagère maison, en formule : +3 €) ──
  { id:'tacos-1-viande', cat:'tacos', name:'Tacos 1 viande', desc:'Galette garnie de frites maison et de sauce fromagère maison, 1 viande au choix.', price:7.00, emoji:'🌯', img:'tacos-1-viande.jpg' },
  { id:'tacos-2-viandes', cat:'tacos', name:'Tacos 2 viandes', desc:'Galette garnie de frites maison et de sauce fromagère maison, 2 viandes au choix.', price:9.00, emoji:'🌯', img:'tacos-2-viandes.jpg', tag:'Populaire', tagClass:'tag-top' },
  ...
  { id:'cheesy-frite-kebab', cat:'cheesy', name:'Cheesy Frite Kebab', desc:'Frites maison, sauce fromagère maison et kebab.', price:7.00, emoji:'🧀', img:'cheesy-frite-kebab.jpg', tag:'Populaire', tagClass:'tag-top' },

  // ── Divers ──
  { id:'divers-riz-viande', cat:'divers', name:'Riz Viande', desc:'Riz maison et viande au choix.', price:10.00, emoji:'🍚', img:'divers-riz-viande.jpg' },
  { id:'divers-riz', cat:'divers', name:'Riz', desc:'Riz maison nature.', price:3.00, emoji:'🍚', img:'divers-riz.jpg' },
  { id:'divers-frites', cat:'divers', name:'Frites Maison', desc:'Bâtonnets de pommes de terre, frites maison.', price:2.50, emoji:'🍟', img:'divers-frites.jpg' },
  { id:'divers-barquette-kebab', cat:'divers', name:'Barquette Kebab', desc:'Lamelles de kebab de veau en barquette.', price:5.00, emoji:'🥩', img:'divers-barquette-kebab.jpg' },

  // ── Menu Enfant ──
  { id:'enfant-menu', cat:'enfant', name:'Menu Enfant', desc:'Nuggets, frites maison et Capri-Sun.', price:7.50, emoji:'🧸', img:'enfant-menu.jpg' },

  // ── Desserts ──
  { id:'desserts-tarte-daim', cat:'desserts', name:'Tarte au Daim', desc:'Tarte gourmande garnie d\'éclats de Daim.', price:3.00, emoji:'🍰', img:'desserts-tarte-daim.jpg' },
  { id:'desserts-tiramisu', cat:'desserts', name:'Tiramisu Maison', desc:'Biscuits imbibés de café, crème mascarpone et cacao. Fait maison.', price:4.00, emoji:'🍮', img:'desserts-tiramisu.jpg', tag:'Populaire', tagClass:'tag-top' },
...
const CAT_NAMES = {
  'sandwichs':'🥙 Nos Sandwichs',
  'burgers':'🍔 Nos Burgers',
  'tacos':'🌯 Tacos',
  'assiettes':'🍽️ Nos Assiettes',
  'cheesy':'🧀 Nos Cheesy Box',
  'divers':'🍟 Divers',
  'enfant':'🧸 Menu Enfant',
  'desserts':'🍰 Desserts',
  'boissons':'🥤 Boissons'
};
const CAT_ORDER = ['sandwichs','burgers','tacos','assiettes','cheesy','divers','enfant','desserts','boissons'];
...
(function applyMenuOverrides(){
  ...
  const sauce1={list:SAUCES_LO,count:1};

  // 1) Sandwichs : sauce au choix + formule (+3 €) + suppléments
  MENU.filter(m=>m.cat==='sandwichs').forEach(m=>{
    m.custom=Object.assign({menu:3.00},m.custom,{sauces:sauce1,extrasConfig:{supps:SUPPS_LO}});
  });
  // Le Mixte : 2 viandes au choix
  if(byId['sandwichs-mixte'])byId['sandwichs-mixte'].custom.viandeChoice=VIANDES_TACOS, byId['sandwichs-mixte'].custom.viandeMax=2;

  // 2) Burgers : sauce au choix + formule (+3 €) + suppléments
  MENU.filter(m=>m.cat==='burgers').forEach(m=>{
    m.custom=Object.assign({menu:3.00},m.custom,{sauces:sauce1,extrasConfig:{supps:SUPPS_LO}});
  });

  // 3) Tacos : viande(s) au choix + sauce + formule (+3 €) + suppléments
  if(byId['tacos-1-viande'])byId['tacos-1-viande'].custom={menu:3.00,viandeChoice:VIANDES_TACOS,viandeMax:1,sauces:sauce1,extrasConfig:{supps:SUPPS_TACOS}};
  if(byId['tacos-2-viandes'])byId['tacos-2-viandes'].custom={menu:3.00,viandeChoice:VIANDES_TACOS,viandeMax:2,sauces:sauce1,extrasConfig:{supps:SUPPS_TACOS}};

  // 4) Assiettes : viande(s) au choix + sauce
  if(byId['assiettes-1-viande'])byId['assiettes-1-viande'].custom={viandeChoice:VIANDES_ASSIETTE,viandeMax:1,sauces:sauce1};
  if(byId['assiettes-2-viandes'])byId['assiettes-2-viandes'].custom={viandeChoice:VIANDES_ASSIETTE,viandeMax:2,sauces:sauce1};

  // 5) Divers : Riz Viande = viande au choix
  if(byId['divers-riz-viande'])byId['divers-riz-viande'].custom={viandeChoice:VIANDES_ASSIETTE,viandeMax:1,sauces:sauce1};
})();

// ═══ STATE ═══════════════════════════════════════════════════
let cart = JSON.parse(localStorage.getItem('loriginal_cart') || '[]');
let activeCat = 'all';
...
function saveCart() { localStorage.setItem('loriginal_cart', JSON.stringify(cart)); }
...
function openModal(itemId, existingCartItem) {
  ...
  if(existingCartItem&&existingCartItem.custom){editState={...existingCartItem.custom};}
    ...
    if(c.menu){editState.menu=false;editState.boissonMenu=DRINKS[0];editState.accompagnement='Frites Maison';}
  }
...
function initCheckout(){
  ...
  const saved=JSON.parse(localStorage.getItem('loriginal_customer')||'{}');
...
async function startPayment(){
  ...
  if($('checkout-remember').checked){localStorage.setItem('loriginal_customer',JSON.stringify({phone,name:$('checkout-name').value.trim()}));}
  ...
  try{
    ...
    if(data.url){
      ...
      localStorage.setItem('loriginal_last_order', JSON.stringify({
...
// ═══ TRACKING ═════════════════════════════════════════════════
function openTrack(){$('track-modal').classList.add('open');$('track-overlay').classList.add('open');lockScroll();const saved=JSON.parse(localStorage.getItem('loriginal_customer')||'{}');if(saved.phone)$('track-phone').value=phonePretty(saved.phone);}
